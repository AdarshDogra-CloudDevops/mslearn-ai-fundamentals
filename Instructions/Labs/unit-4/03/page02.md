...
1. Search for **Storage Accounts** and select **Storage Accounts** from the services. 

1. Now, we need to find the correct storage account from the list of accounts you currently see. 

1. It will be named “**testanomalymod**” followed by a sequence of random numbers. For example, **testanomalymod7380583022**. Once you find it, click on that account.

1. From here, we’ll click on **Containers (1)** from the left panel. After clicking containers, click on the blob storage that starts with **azureml-blobstore-xxxxxx (2)**.
